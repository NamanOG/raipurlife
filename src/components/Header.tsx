...
const Header = ({ overlay = false }: HeaderProps) => {
  ...
  const frameClass = overlay
    ? "flex items-center justify-between gap-3 rounded-[1.25rem] border border-slate-700/55 bg-slate-950/42 px-3 py-1.5 shadow-[0_14px_30px_hsl(217_30%_6%_/_0.26)] backdrop-blur-xl"
      ...
      <div className="container mx-auto px-4 py-2">
        <div className={frameClass}>
          <Link to="/" className="group flex items-center gap-3">
            <div className="h-9 w-9 overflow-hidden rounded-xl shadow-md transition-transform duration-300 group-hover:-rotate-6 lg:h-10 lg:w-10">
          ...
          <nav className="hidden items-center gap-1 lg:flex">
            {NAV_LINKS.map((link) => (
              <NavLink
                ...
                to={link.href}
                className={({ isActive }) =>
                  `relative px-3 py-1.5 text-sm font-semibold tracking-wide transition-[background-color,color,transform,box-shadow] duration-300 motion-safe:hover:-translate-y-0.5 ${
                    isActive
                      ? overlay
